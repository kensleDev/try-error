...
This utility module encourages a pattern of handling asynchronous and synchronous operations through predictable tuples instead of relying solely on try/catch blocks.

**It is NOT a "never throw" approach** — exceptions are still allowed and meaningful. Instead, thrown errors are captured and surfaced as the second value of a result tuple, allowing safe branching and structured error handling without suppressing exceptions.

Similar to Rust's `Result<T, E>`, we model results as `[T, null]` or `[null, Error]`, enabling developers to destructure outcomes, check for failure via helpers, and respond clearly.

You can still throw when needed. This pattern simply ensures the caller can handle it without crashing.
